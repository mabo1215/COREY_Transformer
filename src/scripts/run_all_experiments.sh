...
for subset in narrativeqa qasper gov_report multifieldqa_en; do
	python3 src/experiments/run_external_baselines.py --models rwkv flashattention mamba2 \
		--data-file src/data/longbench_subset/$subset/test.jsonl --device cuda
done

# 2. Quamba 量化推理基准 (LongBench四子集)
for subset in narrativeqa qasper gov_report multifieldqa_en; do
	python3 src/experiments/run_quamba_quant_benchmark.py --model-id benchang1110/mamba2-2.7b-hf \
...
for subset in narrativeqa qasper gov_report multifieldqa_en; do
	python3 src/experiments/run_policy_corey_ablation.py --model-id benchang1110/mamba2-2.7b-hf \
		--n 20 --policy corey \
		--data-file src/data/longbench_subset/$subset/test.jsonl --device cuda
done
