...
def run_benchmark(args: argparse.Namespace) -> dict[str, Any]:
    ...
    off_mean = off_result["latency_mean_ms"]
    static_mean = static_result["latency_mean_ms"]
    corey_mean = corey_result["latency_mean_ms"]

    comparison: list[dict[str, Any]] = []
    for r in results:
        speedup_vs_off = round(off_mean / r["latency_mean_ms"], 3) if r["latency_mean_ms"] > 0 else None
        speedup_vs_static = round(static_mean / r["latency_mean_ms"], 3) if r["latency_mean_ms"] > 0 else None
        comparison.append({
            "policy": r["policy"],
    ...
    print(f"  policy_off     : {off_mean:.1f} ms  (Python loop, {off_result['n_chunks']} iterations)")
    print(f"  policy_static  : {static_mean:.1f} ms  (chunk={static_result['chunk_size']}, "
          f"{static_result['n_chunks']} kernel calls)  "
          f"{comparison[1]['speedup_vs_off']:.2f}x vs off")
    print(f"  policy_corey   : {corey_mean:.1f} ms  (chunk={corey_result['chunk_size']}, "
          f"{corey_result['n_chunks']} kernel calls)  "
          f"{comparison[2]['speedup_vs_off']:.2f}x vs off, "
          f"{comparison[2]['speedup_vs_static']:.2f}x vs static")
